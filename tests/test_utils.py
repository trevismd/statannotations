...
from statannotations.utils import raise_expected_got, check_is_in, check_not_none


class TestUtils(unittest.TestCase):
    def test_raise_expected_got(self):
        with self.assertRaisesRegex(ValueError, "Expected `this`; got `that` instead."):
            raise_expected_got("this", None, "that")

    def test_raise_expected_got_for(self):
        with self.assertRaisesRegex(ValueError, "Expected `this` for 'this_variable'; got `that` instead."):
